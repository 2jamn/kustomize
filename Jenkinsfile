pipeline {
  ...
  environment {
    ...
    awsecrRegistry = '851725651998.dkr.ecr.ap-northeast-2.amazonaws.com/eks-demo-repo'
    awsecrRegistryCredential = 'credential-AWS-ECR'
    githubCredential = 'credential-github'
    gitEmail = 'aschente@naver.com'
    gitName = '2jamn'
  }
  ...
  stages {
    ...
    stage('Checkout Application Git Branch') {
      steps {
        checkout([$class: 'GitSCM', branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: githubCredential, url: 'https://github.com/2jamn/kustomize.git']]])
      }
      // steps 가 끝날 경우 실행한다.
    ...
    stage('Deploy') { 
      steps {
        ...
        git credentialsId: githubCredential,
            url: 'https://github.com/2jamn/kustomize.git',
            branch: 'main'  
